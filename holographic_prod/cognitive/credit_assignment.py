"""
Credit Assignment v2 — Lean Integration with HolographicMemory
=====================================================================
...
from holographic_prod.core.constants import (
    PHI_INV, PHI_INV_SQ, PHI_INV_CUBE, PHI_INV_FOUR,
)
from holographic_prod.core.algebra import geometric_product
...
class CreditAssignmentTracker:
    """
    Lean credit assignment for HolographicMemory.
    ...
    def record_error(
        ...
        predicted: int,
        actual: int,
        confidence: float = PHI_INV_SQ,  # φ-derived: φ⁻² ≈ 0.382
    ):
        """
        ...
        Args:
            ...
            confidence: Confidence of the wrong prediction (default: φ⁻² ≈ 0.382)
        """
        if predicted == actual:
            return  # Not an error
    ...
    def _reconsolidate_single(self, error: ErrorRecord) -> Dict[str, bool]:
        ...
        THEORY:
            When memory predicts A but correct answer is B:
        ...
        IMPLEMENTATION (v4.30):
            HolographicMemory uses holographic superposition + Grace basins.
            We apply reconsolidation directly to the holographic_memory matrix.
        """
        # Get context matrix (GPU-aware)
        ctx_mat = self.memory.embed_sequence(list(error.context), return_gpu=self.memory.use_gpu)
        
        # Get embeddings (convert to GPU if needed)
        actual_embed = self.memory.embed(error.actual)
        predicted_embed = self.memory.embed(error.predicted)
        
        # Ensure both arrays are on the same device
        if self.memory.use_gpu:
            import cupy as cp
            if not isinstance(actual_embed, cp.ndarray):
                actual_embed = cp.asarray(actual_embed)
            if not isinstance(predicted_embed, cp.ndarray):
                predicted_embed = cp.asarray(predicted_embed)
            if not isinstance(ctx_mat, cp.ndarray):
                ctx_mat = cp.asarray(ctx_mat)
        
        # Compute correction bindings (geometric product is just matrix multiplication)
        correct_binding = geometric_product(ctx_mat, actual_embed)
        wrong_binding = geometric_product(ctx_mat, predicted_embed)
        ...
        # Apply to holographic memory (superposition-based)
        # Ensure holographic_memory is on the same device
        if self.memory.use_gpu:
            import cupy as cp
            if not isinstance(self.memory.holographic_memory, cp.ndarray):
                self.memory.holographic_memory = cp.asarray(self.memory.holographic_memory)
        
        # BOOST: Add correct binding
        self.memory.holographic_memory += boost_rate * correct_binding
        # ATTENUATE: Subtract wrong binding
        self.memory.holographic_memory -= attenuate_rate * wrong_binding
