...
# NOTE: Functions use string type hints 'TheoryTrueModel' for documentation.
# In practice, any object with .xp, .basis, .embed(), .embed_sequence() works.
# HolographicMemory is the current production implementation.
...
class AgentModelBuilder:
    ...
    def observe(
        ...
        NOTE: Vorticity is computed from accumulated observations in build(),
        not per-observation, since vorticity_signature requires sequences.
        """
        ...
        # NOTE: Vorticity computed in build() from accumulated observations
    ...
    def build(self) -> AgentModel:
        ...
        # Compute vorticity pattern from accumulated observations
        # vorticity_signature expects [n, 4, 4] sequence
        vorticity_pattern = None
        if len(self.observations) >= 2:
            # Stack observations into a sequence
            obs_stack = xp.stack(self.observations)  # [n, 4, 4]
            vorticity_pattern = vorticity_signature(obs_stack, self.basis, xp)
...
def explain_other_action(
    ...
    action_perception_sim = frobenius_cosine(action, their_perception, xp)
