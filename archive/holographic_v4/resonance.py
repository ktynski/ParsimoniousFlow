...
DISTRIBUTED PRIOR (Brain-Analog Population Coding):
    ...
    6. Use geometric confidence (margin) to gate fallback
...
def grace_basin_discovery(
    ...
    query_witness_norm = abs(query_witness[0]) + abs(query_witness[1]) + 1e-8
    ...
    for i, proto in enumerate(prototypes):
        ...
        if use_vorticity:
            ...
            if proto_vort is not None:
                ...
                if query_vort_norm > 1e-8 and proto_vort_norm > 1e-8:
...
def distributed_prior_retrieve(
    ...
    Args:
        ...
        factorized_prior: Optional global fallback (FactorizedAssociativePrior)
...
class TheoryTrueRetriever:
    ...
    def __init__(
        ...
        # Global fallback (factorized prior)
    ...
    def init_factorized_prior(self, witness_dim: int = 4):
        """
        Initialize the factorized associative prior (global fallback).
        
        Call this after creating the retriever to enable global fallback
        for queries in uncovered regions.
        """
        from holographic_v4.distributed_prior import FactorizedAssociativePrior
