...
def extract_object_from_bound(
    ...
    reduced = bound - 0.5 * estimated_wedge  # Partial removal
...
def compare_bindings(
    ...
    # Overall similarity
    result['overall'] = float(frobenius_similarity(binding1, binding2, xp))
...
def binding_similarity(
    ...
    if grade_weights is None:
        # Default: weight grade 2 higher (that's where binding info lives)
        grade_weights = {0: 0.1, 1: 0.2, 2: 0.4, 3: 0.2, 4: 0.1}
